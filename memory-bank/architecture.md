...
Dashboard visual assets live inside the plugin bundle under `assets/`. The desktop banner is `assets/frontcover.png`, and the mobile lightweight dashboard banner is `assets/mobilecover.png`; both are resolved through `app.vault.adapter.getResourcePath()` from the plugin directory. Do not rely on external URLs, vault-relative `<img src>` paths, or root-level loose image files for Dashboard chrome.
