...
- `src/modals/PracticeLogModal.ts`: native Obsidian modal for recording practice sessions with collection binding, module, totals, wrong count, duration, round, and range fields. It also contains the user-facing entry for creating a new practice collection, keeping "新增专题" out of the top toolbar.
...
Dashboard visual assets are source PNG files under `assets/`, with `assets/frontcover.png` for desktop and `assets/mobilecover.png` for mobile. Production builds import these PNG files through esbuild's data URL loader, embedding them into `main.js` so Community Plugin installs still show the bitmap banners when only `main.js`, `manifest.json`, and `styles.css` are present. If a local/manual install includes the `assets/` directory, `src/main.ts` prefers `app.vault.adapter.getResourcePath()` for the on-disk image and falls back to the embedded data URL when the file is missing. Do not rely on external URLs, vault-relative `<img src>` paths, or root-level loose image files for Dashboard chrome.
